...
export function CallModal({ callSession, callService, isIncoming, currentUser, onClose }: Props) {
  ...
  // Incoming Call Full-Screen View
  if (isIncoming && callStatus === 'ringing') {
    return (
      <div className="fixed inset-0 z-50 bg-zinc-950 flex flex-col items-center justify-between p-6 sm:p-12 overflow-hidden select-none">
        {/* Top Header */}
        <div className="w-full max-w-4xl flex items-center justify-between z-20">
          <div className="flex items-center gap-3">
            <div className="w-3 h-3 rounded-full bg-emerald-500 animate-ping"></div>
            <div>
              <h2 className="text-white font-semibold text-base sm:text-lg">SoundLink Secure Call</h2>
              <p className="text-xs text-zinc-400">Encrypted P2P HD Connection</p>
            </div>
          </div>
        ...
        {/* Center Caller Showcase */}
        <div className="flex flex-col items-center justify-center gap-6 my-auto z-20 text-center">
          <div className="relative">
            <div className="absolute -inset-8 rounded-full bg-emerald-500/20 animate-ping"></div>
            ...
            <img
              ...
              className="w-36 h-36 sm:w-44 sm:h-44 rounded-full object-cover border-4 border-emerald-500 relative z-10 shadow-2xl"
            />
          </div>

          <div>
            <h3 className="text-2xl sm:text-3xl font-bold text-white mb-2">{otherUser?.name || 'Incoming Caller'}</h3>
            <p className="text-zinc-400 text-sm sm:text-base">@{otherUser?.username || 'contact'}</p>
          </div>
          ...
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs sm:text-sm font-semibold tracking-wide">
            {isVideo ? <VideoIcon className="w-4 h-4" /> : <Phone className="w-4 h-4" />}
            Incoming HD {isVideo ? 'Video' : 'Voice'} Call...
          </div>
        </div>

        {/* Bottom Full-Width Action Buttons */}
        <div className="w-full max-w-md flex items-center justify-around gap-6 z-20 pb-6">
          <button
            onClick={handleDecline}
            className="flex flex-col items-center gap-3 group"
          >
            <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-red-600 hover:bg-red-500 text-white flex items-center justify-center shadow-xl shadow-red-600/40 transition-all group-hover:scale-110 active:scale-95">
              <PhoneOff className="w-7 h-7 sm:w-8 sm:h-8" />
            </div>
            <span className="text-xs sm:text-sm text-zinc-400 group-hover:text-red-400 font-semibold tracking-wide">Decline</span>
          </button>
          ...
          <button
            ...
            className="flex flex-col items-center gap-3 group"
          >
            <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-emerald-500 hover:bg-emerald-400 text-black flex items-center justify-center shadow-xl shadow-emerald-500/40 transition-all group-hover:scale-110 active:scale-95 animate-pulse">
              {isVideo ? <VideoIcon className="w-7 h-7 sm:w-8 sm:h-8" /> : <Phone className="w-7 h-7 sm:w-8 sm:h-8" />}
            </div>
            <span className="text-xs sm:text-sm text-emerald-400 font-bold tracking-wide">Accept</span>
          </button>
        </div>
      </div>
    );
